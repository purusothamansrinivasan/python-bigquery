...
func Bigquery_models_deleteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateBigquery_models_deleteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_projects_projectId_datasets_datasetId_models_modelId",
		mcp.WithDescription("Deletes the model specified by modelId from the dataset."),
		mcp.WithString("projectId", mcp.Required(), mcp.Description("Required. Project ID of the model to delete.")),
		mcp.WithString("datasetId", mcp.Required(), mcp.Description("Required. Dataset ID of the model to delete.")),
		mcp.WithString("modelId", mcp.Required(), mcp.Description("Required. Model ID of the model to delete.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Bigquery_models_deleteHandler(cfg),
	}
}
