...
func Bigquery_models_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateBigquery_models_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_projectId_datasets_datasetId_models_modelId",
		mcp.WithDescription("Gets the specified model resource by model ID."),
		mcp.WithString("projectId", mcp.Required(), mcp.Description("Required. Project ID of the requested model.")),
		mcp.WithString("datasetId", mcp.Required(), mcp.Description("Required. Dataset ID of the requested model.")),
		mcp.WithString("modelId", mcp.Required(), mcp.Description("Required. Model ID of the requested model.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Bigquery_models_getHandler(cfg),
	}
}
