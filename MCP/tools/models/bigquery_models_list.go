...
func Bigquery_models_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/projects/%s/datasets/%s/models%s", cfg.BaseURL, projectId, datasetId, queryString)
...
func CreateBigquery_models_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_projectId_datasets_datasetId_models",
		mcp.WithDescription("Lists all models in the specified dataset. Requires the READER dataset role. After retrieving the list of models, you can get information about a particular model by calling the models.get method."),
		mcp.WithString("projectId", mcp.Required(), mcp.Description("Required. Project ID of the models to list.")),
		mcp.WithString("datasetId", mcp.Required(), mcp.Description("Required. Dataset ID of the models to list.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Bigquery_models_listHandler(cfg),
	}
}
