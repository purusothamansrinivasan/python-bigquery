...
func Bigquery_jobs_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/projects/%s/jobs/%s%s", cfg.BaseURL, projectId, jobId, queryString)
...
func CreateBigquery_jobs_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_projectId_jobs_jobId",
		mcp.WithDescription("Returns information about a specific job. Job information is available for a six month period after creation. Requires that you're the person who ran the job, or have the Is Owner project role."),
		mcp.WithString("projectId", mcp.Required(), mcp.Description("Required. Project ID of the requested job.")),
		mcp.WithString("jobId", mcp.Required(), mcp.Description("Required. Job ID of the requested job.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Bigquery_jobs_getHandler(cfg),
	}
}
