...
func Bigquery_datasets_insertHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateBigquery_datasets_insertTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_projects_projectId_datasets",
		mcp.WithDescription("Creates a new empty dataset."),
		mcp.WithString("projectId", mcp.Required(), mcp.Description("Required. Project ID of the new dataset")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Bigquery_datasets_insertHandler(cfg),
	}
}
