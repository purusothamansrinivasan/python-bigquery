...
func Bigquery_datasets_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateBigquery_datasets_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_projects_projectId_datasets_datasetId",
		mcp.WithDescription("Updates information in an existing dataset. The update method replaces the entire dataset resource, whereas the patch method only replaces fields that are provided in the submitted dataset resource."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Bigquery_datasets_updateHandler(cfg),
	}
}
