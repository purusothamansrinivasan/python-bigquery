...
func Bigquery_routines_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var result models.ListRoutinesResponse
...
func CreateBigquery_routines_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_projects_projectId_datasets_datasetId_routines",
		mcp.WithDescription("Lists all routines in the specified dataset. Requires the READER dataset role."),
		mcp.WithString("projectId", mcp.Required(), mcp.Description("Required. Project ID of the routines to list")),
		mcp.WithString("datasetId", mcp.Required(), mcp.Description("Required. Dataset ID of the routines to list")),
		mcp.WithString("filter", mcp.Description("If set, then only the Routines matching this filter are returned. The supported format is `routineType:{RoutineType}`, where `{RoutineType}` is a RoutineType enum. For example: `routineType:SCALAR_FUNCTION`.")),
		...
		mcp.WithString("readMask", mcp.Description("If set, then only the Routine fields in the field mask, as well as project_id, dataset_id and routine_id, are returned in the response. If unset, then the following Routine fields are returned: etag, project_id, dataset_id, routine_id, routine_type, creation_time, last_modified_time, and language.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Bigquery_routines_listHandler(cfg),
	}
}
