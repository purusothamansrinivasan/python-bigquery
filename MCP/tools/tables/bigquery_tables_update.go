...
func Bigquery_tables_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateBigquery_tables_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_projects_projectId_datasets_datasetId_tables_tableId",
		mcp.WithDescription("Updates information in an existing table. The update method replaces the entire Table resource, whereas the patch method only replaces fields that are provided in the submitted Table resource."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Bigquery_tables_updateHandler(cfg),
	}
}
