...
func Bigquery_tables_insertHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateBigquery_tables_insertTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_projects_projectId_datasets_datasetId_tables",
		mcp.WithDescription("Creates a new, empty table in the dataset."),
		mcp.WithString("projectId", mcp.Required(), mcp.Description("Required. Project ID of the new table")),
		mcp.WithString("datasetId", mcp.Required(), mcp.Description("Required. Dataset ID of the new table")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Bigquery_tables_insertHandler(cfg),
	}
}
